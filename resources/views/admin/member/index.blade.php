    ...
    <div class="">
      <div class="page-title">
        ...
        <div class="col-auto float-right ml-auto">
          <a href="{{ route('addMember') }}" class="btn add-btn btn-primary" style="background-color: #1a2eb9; border: 1px solid #1a2eb9;color: #fff; margin-right: 7px; border-radius:10px; margin-top: 10px;"  ><i class="fa fa-plus"></i> Add Member</a>
      </div>

        <div class="title_right">
          <div class="col-md-5 col-sm-5 col-xs-12 form-group pull-right top_search">
            <div class="input-group">
                <button type="button" class="btn btn-primary"><a href="{{route('addMember')}}" style="color:white">Add Member</a></button>
            </div>
          </div>
        </div>
      </div>
              ...
              <table id="slider-datatable" class="table table-striped table-bordered" style="width:100%">
                ...
                <tbody>
                  ...
                  <tr>
                    <td>{{$member->id}}</td>
                    <td>{{$member->name}}</td>
                    <td><img src="{{asset('public/uploads/member/'.$member->image)}}" style="height:80px;width:80px;"></td>
                    <td>{{$member->address}}</td>
                    <td>{{$member->officename}}</td>
                    <td>{{$member->desc}}</td>
                    
                    
                    <td><a href="{{route('editMember',$member->id)}}" data-toggle="tooltip" title="Edit" class="btn btn-sm btn-outline-primary"><i class="fa fa-edit"></i></a>
                        <a href="{{route('deleteMember',$member->id)}}" data-toggle="tooltip" title="Delete" class="btn btn-sm btn-outline-danger"><i class="fa fa-trash-o"></i></a>
                    </td>
